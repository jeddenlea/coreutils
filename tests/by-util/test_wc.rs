...
macro_rules! not_found {
    ($m:literal) => {
        concat!("wc: ", $m, ": No such file or directory\n")
    };
}
...
macro_rules! not_found {
    ($m:literal) => {
        concat!("wc: ", $m, ": The system cannot find the file specified\n")
    };
}
...
fn test_read_from_nonexistent_file() {
    new_ucmd!()
        .args(&["bogusfile"])
        .fails()
        .stderr_only(not_found!("bogusfile"));
}
...
fn test_files0_errors_quoting() {
    new_ucmd!()
        ...
        .failure()
        .stderr_is(concat!(
            not_found!("this_file_does_not_exist.txt"),
            "wc: 'files0 with nonexistent.txt':2: invalid zero-length file name\n",
            not_found!("'this file does not exist.txt'"),
            not_found!("\"this files doesn't exist either.txt\""),
        ))
